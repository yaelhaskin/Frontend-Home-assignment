...
const CauseSelection = ({ causes }) => {
    ...
    const handleSelectedCause = (selected) => {
        if(selectedCauses.includes(selected)) {
            setSelectedCauses((prev) => prev.filter((cause) => cause !== selected));
        } else {
            if(selectedCauses.length === MAX_SELECT) {
                setShowPopup(`You can add only 3 causes.\n\nTo add a different cause, first remove one.`);
            } else {
                setSelectedCauses((prev) => [...prev, selected]);
            }
        }
    }
        ...
        <div className={classes.CauseSelection}>
            <div className={classes.gridContainer}>
                ...
                <div className={classes.progress}>
                    ...
                    />
                    <Link href="/sign-up" className={`button ${classes.continueBtn}`} onClick={handleContinueBtn}>
                        Continue
